...
function id_check(){
	...
	if(!isUserid(idf.userid)){
		alert('아이디는 영문자, 숫자,_,!로 4~8자리까지 가능해요');
		idf.userid.select();
		return;
	}
	idf.submit();
...
function member_check() {
   ...
   if (!isUserid(mf.userid)) {
      alert('아이디는 영문자, 숫자, _, !만 사용 가능해요');
      mf.userid.select();
      return;
   }
   if (!isPasswd(mf.pwd)) {
      alert('비밀번호는 문자, 숫자, !, .을 포함해서 4~8자리 이내로 입력하세요');
      return;
   }
...
function isUserid(input) {
	let val=input.value;
	let pattern=/^[a-zA-Z0-9_!]{4,8}$/; //a or b or c // /abc/=> only abc
	let b=pattern.test(val);
	//alert(b);
	return b;
}
